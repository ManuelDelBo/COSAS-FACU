//Manuel Ignacio Del Bo - Comisión 14
...
{
    //EJ 1:
    char tiposMuestras[DIM];
    char concentracionesMuestras[DIM];
    int muestras = 8;

    cargaArreglos(tiposMuestras, concentracionesMuestras, muestras);
    ...
    clasificaConcentraciones(tiposMuestras, concentracionesMuestras, muestras, &pilaSeguras, &pilaCriticas, &pilaTipo1);

    //EJ 3:
    int umbral = 45;

    informaPromPorc(concentracionesMuestras, muestras, pilaSeguras, umbral);

    //EJ 4:
    char matPersonas[APYNOM][CARACTERES];

    int cantPersonas = cargaApeyNom(matPersonas);
    int posiciones[cantPersonas];
    int cantidad = buscaApe(matPersonas, cantPersonas, posiciones);

    char matEncontrados[APYNOM][CARACTERES];

    cargaBuscados(matEncontrados, matPersonas, posiciones, cantidad);
